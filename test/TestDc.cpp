...
namespace rosneuro {
    ...
    TEST_F(DcTestSuite, Integration) {
        ASSERT_TRUE(dc->configure());

        int frame_size = 32;
        std::string base_path = ros::package::getPath("rosneuro_filters_dc");

        const std::string input_path = base_path + "/test/rawdata.csv";
        const std::string output_path = base_path + "/test/expected.csv";

        rosneuro::DynamicMatrix<double> input = readCSV<double>(input_path);
        rosneuro::DynamicMatrix<double> expected = readCSV<double>(output_path);
        ...
        rosneuro::DynamicMatrix<double> frame = rosneuro::DynamicMatrix<double>::Zero(frame_size, nchannels);

        for(auto i = 0; i<nsamples; i = i+frame_size) {
            frame = input.middleRows(i, frame_size);
            output.middleRows(i, frame_size) = dc->apply(frame);
        }
        ASSERT_TRUE(expected.isApprox(output, 1e-6));
    }
}
